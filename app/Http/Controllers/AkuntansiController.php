...
class AkuntansiController extends Controller
{
    ...
    public function bo_ak_tr_postingtransaksi(Request $request)
    {
        ...
        $sqlpostdep = "SELECT deptrans.DEPTRANS_ID,deptrans.TGL_TRANS,deptrans.NO_REKENING,deptrans.SALDO_TRANS,deptrans.KODE_TRANS,deptrans.MY_KODE_TRANS,perkiraan.dk as TYPE_TRANS,deptrans.KUITANSI,kodetransdeposito.GL_TRANS,kodejenisdeposito.KODE_PERK,kodejenisdeposito.KODE_PERK_BIAYA,kodejenisdeposito.KODE_PERK_TITIPAN,deposito.MASUK_TITIPAN,deposito.NO_REK_TABUNGAN,deptrans.NO_REK_OB,kodejenistabungan.KODE_PERK as PERK_TAB,deposito.BUNGA_BERBUNGA FROM (((((deptrans INNER JOIN kodetransdeposito ON deptrans.KODE_TRANS=kodetransdeposito.KODE_TRANS) INNER JOIN deposito ON deptrans.NO_REKENING=deposito.NO_REKENING) INNER JOIN kodejenisdeposito ON deposito.JENIS_DEPOSITO=kodejenisdeposito.KODE_JENIS_DEPOSITO) LEFT JOIN tabung ON deptrans.NO_REK_OB=tabung.NO_REKENING) LEFT JOIN kodejenistabungan ON tabung.JENIS_TABUNGAN=kodejenistabungan.KODE_JENIS_TABUNGAN) LEFT JOIN perkiraan ON kodetransdeposito.GL_TRANS=perkiraan.kode_perk WHERE deptrans.POSTED=0 AND (deptrans.TGL_TRANS>='$request->tgl_awal' AND deptrans.TGL_TRANS<='$request->tgl_akhir') ORDER BY deptrans.DEPTRANS_ID";
        ...
        for ($i = 0; $i < count($postingdep); $i++) {
            ...
            // POSTING BUNGA DIAMBIL DARI TELLER SECARA TUNAI KE TRANS_MASTER BUFFER 
            elseif ($postingdep[$i]->MY_KODE_TRANS == '200' and empty($postingdep[$i]->NO_REK_OB) == true and (substr($postingdep[$i]->KUITANSI, 0, 7) == substr($postingdep[$i + 1]->KUITANSI, 0, 7))) {
                $inputtransmasterbuff->tgl_trans = $postingdep[$i]->TGL_TRANS;
                $inputtransmasterbuff->kode_jurnal = 'KK';
                $inputtransmasterbuff->no_bukti = $postingdep[$i]->KUITANSI;
                $inputtransmasterbuff->nominal = $postingdep[$i]->SALDO_TRANS;
                $inputtransmasterbuff->keterangan = 'Penarikan Tunai Bunga Deposito #Rek' . $postingdep[$i]->NO_REKENING;
                $inputtransmasterbuff->save();
            }
            // POSTING BUNGA DIAMBIL DARI TELLER SECARA OB TABUNGAN KE TRANS_MASTER BUFFER 
            elseif ($postingdep[$i]->MY_KODE_TRANS == '200' and empty($postingdep[$i]->NO_REK_OB) == false and (substr($postingdep[$i]->KUITANSI, 0, 7) == substr($postingdep[$i + 1]->KUITANSI, 0, 7))) {
                $inputtransmasterbuff->tgl_trans = $postingdep[$i]->TGL_TRANS;
                $inputtransmasterbuff->kode_jurnal = 'KK';
                $inputtransmasterbuff->no_bukti = $postingdep[$i]->KUITANSI;
                $inputtransmasterbuff->nominal = $postingdep[$i]->SALDO_TRANS;
                $inputtransmasterbuff->keterangan = 'OB Penarikan Bunga Deposito #Rek' . $postingdep[$i]->NO_REKENING . ' ke #RekTab :' . $postingdep[$i]->NO_REK_OB;
                $inputtransmasterbuff->save();
            }
            // POSTING PENUTUPAN DEPOSITO LEWAT TABUNGAN 
            elseif ($postingdep[$i]->MY_KODE_TRANS == '300' and empty($postingdep[$i]->NO_REK_OB) == false) {
                $inputtransmasterbuff->tgl_trans = $postingdep[$i]->TGL_TRANS;
                $inputtransmasterbuff->kode_jurnal = 'KK';
                $inputtransmasterbuff->no_bukti = $postingdep[$i]->KUITANSI;
                $inputtransmasterbuff->nominal = $postingdep[$i]->SALDO_TRANS;
                $inputtransmasterbuff->keterangan = 'OB/PB Penutupan Deposito #Rek' . $postingdep[$i]->NO_REKENING . ' ke #RekTab :' . $postingdep[$i]->NO_REK_OB;
                $inputtransmasterbuff->save();
            }
            // POSTING PENUTUPAN DEPOSITO TUNAI 
            elseif ($postingdep[$i]->MY_KODE_TRANS == '300' and empty($postingdep[$i]->NO_REK_OB) == true) {
                $inputtransmasterbuff->tgl_trans = $postingdep[$i]->TGL_TRANS;
                $inputtransmasterbuff->kode_jurnal = 'KK';
                $inputtransmasterbuff->no_bukti = $postingdep[$i]->KUITANSI;
                $inputtransmasterbuff->nominal = $postingdep[$i]->SALDO_TRANS;
                $inputtransmasterbuff->keterangan = 'Tunai Penutupan Deposito #Rek' . $postingdep[$i]->NO_REKENING;
                $inputtransmasterbuff->save();
            }
            ...
            elseif (empty($inputtransmasterbuff->trans_id) == false and $postingdep[$i]->MY_KODE_TRANS == '0' and empty($postingdep[$i]->NO_REK_OB) == false) {

                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $inputtransmasterbuff->trans_id;
                $inputtransdetailbuff->URAIAN = 'Buka Deposito #REK' . $norek[$i] . 'via Tabungan #Rek ' . $postingdep[$i]->NO_REK_OB;
                $inputtransdetailbuff->kode_perk = $postingdep[$i]->PERK_TAB;
                $inputtransdetailbuff->debet = $saldotrans[$i];
                $inputtransdetailbuff->save();
                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $inputtransmasterbuff->trans_id;
            ...
            // AMBIL TITIPAN 
            // TUNAI
            elseif (empty($inputtransmasterbuff->trans_id) == false and substr($postingdep[$i]->KUITANSI, 0, 3) <> "SYS" and $postingdep[$i]->TYPE_TRANS == 'D' and $postingdep[$i]->MASUK_TITIPAN == 1 and empty($postingdep[$i]->NO_REK_OB) == true and $postingdep[$i]->MY_KODE_TRANS <> '200') {
            ...
            elseif (empty($inputtransmasterbuff->trans_id) == false and substr($postingdep[$i]->KUITANSI, 0, 3) <> "SYS" and $postingdep[$i]->TYPE_TRANS == 'D' and $postingdep[$i]->MASUK_TITIPAN == 1 and empty($postingdep[$i]->NO_REK_OB) == false and ($postingdep[$i]->MY_KODE_TRANS <> 200 || $postingdep[$i]->MY_KODE_TRANS <> 400)) {
            ...
            // SIMPAN PENGAMBILAN BUNGA TUNAI LEWAT TELLER ke TRANS_DETAIL
            elseif ($i > 0 and $postingdep[$i - 1]->MY_KODE_TRANS == '200' and $postingdep[$i]->MY_KODE_TRANS == '400' and empty($postingdep[$i]->NO_REK_OB) == true and (substr($postingdep[$i]->KUITANSI, 0, 7) ==  substr($postingdep[$i - 1]->KUITANSI, 0, 7))) {
                // dd('9');
                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $ambiltrans_id[$i - 1];
                $inputtransdetailbuff->URAIAN = 'OB/Tunai Penarikan Bunga Deposito ' . $norek[$i];
                $inputtransdetailbuff->kode_perk = $postingdep[$i]->KODE_PERK;
                $inputtransdetailbuff->debet = $saldotrans[$i - 1];
                $inputtransdetailbuff->save();
                $inputtransdetailbuff = new Trans_detail_buffer();
            ...
            // SIMPAN PENGAMBILAN BUNGA KETABUNGAN LEWAT TELLER ke TRANS_DETAIL

            elseif ($i > 0 and $postingdep[$i - 1]->MY_KODE_TRANS == '200' and $postingdep[$i]->MY_KODE_TRANS == '400' and empty($postingdep[$i]->NO_REK_OB) == false and (substr($postingdep[$i]->KUITANSI, 0, 7) ==  substr($postingdep[$i - 1]->KUITANSI, 0, 7))) {
                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $ambiltrans_id[$i - 1];
                $inputtransdetailbuff->URAIAN = 'OB Penarikan Bunga Deposito ' . $norek[$i] . ' ke tabungan';
                $inputtransdetailbuff->kode_perk = $postingdep[$i]->KODE_PERK;
                $inputtransdetailbuff->debet = $saldotrans[$i - 1];
                $inputtransdetailbuff->save();
                $inputtransdetailbuff = new Trans_detail_buffer();
            ...
            // SIMPAN PENUTUPAN DEPOSITO TUNAI ke TRANS_DETAIL
            elseif ($i > 0 and empty($inputtransmasterbuff->trans_id) == false and $postingdep[$i]->MY_KODE_TRANS == '300' and empty($postingdep[$i]->NO_REK_OB) == true) {
                // dd('11');
                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $inputtransmasterbuff->trans_id;
                $inputtransdetailbuff->URAIAN = 'OB/Tunai Penutupan Deposito #Rek :' . $norek[$i];
                $inputtransdetailbuff->kode_perk = $postingdep[$i]->KODE_PERK;
                $inputtransdetailbuff->debet = $saldotrans[$i];
                $inputtransdetailbuff->save();
                $inputtransdetailbuff = new Trans_detail_buffer();
            ...
            // SIMPAN PENUTUPAN DEPOSITO OB TABUNGAN ke TRANS_DETAIL
            elseif ($i > 0 and empty($inputtransmasterbuff->trans_id) == false and $postingdep[$i]->MY_KODE_TRANS == '300' and empty($postingdep[$i]->NO_REK_OB) == false) {

                $inputtransdetailbuff = new Trans_detail_buffer();
                $inputtransdetailbuff->master_id = $inputtransmasterbuff->trans_id;
                $inputtransdetailbuff->URAIAN = 'OB/Tunai Penutupan Deposito #Rek :' . $norek[$i] . ' Ke Tabungan';
                $inputtransdetailbuff->kode_perk = $postingdep[$i]->KODE_PERK;
                $inputtransdetailbuff->debet = $saldotrans[$i];
                $inputtransdetailbuff->save();
                $inputtransdetailbuff = new Trans_detail_buffer();
    ...
    public function bo_ak_tt_showfrmctttransaksi()
    {
        ...
        return view('akuntansi.frmcatatjurnaltransaksi', ['logos' => $logos, 'users' => $users, 'perkiraan' => $perkiraan, 'kodejurnal' => $kodejurnal, 'hasil' => $hasil, 'masterid' => '', 'tgllogin' => $tgllogin, 'msgstatus' => '']);
    }
    // Simpan Jurnal Sementara sebelum di simpan ke Trans_master dan trans_detail
    public function bo_ak_tt_savetempjurnalmemorial(Request $request)
    {
        $this->validate($request, [
            'kode_perk' => 'required'
        ]);
    ...
    public function bo_ak_tt_delcatatjurnaldetail(Request $request)
    {
        ...
        return view('akuntansi.frmcatatjurnaltransaksi', ['logos' => $logos, 'users' => $users, 'perkiraan' => $perkiraan, 'kodejurnal' => $kodejurnal, 'hasil' => $hasil, 'masterid' => $master_id, 'tgl_trans' => $tgl_trans, 'kode_jurnal' => $kode_jurnal, 'no_bukti' => $no_bukti, 'keterangan' => $keterangan, 'total' => $total, 'tgllogin' => $tgllogin, 'msgstatus' => '']);
    }
    ...
    public function bo_ak_tt_historycatatjurnal()
    {
        ...
        $logos = Logo::all();
        $history = [];
        $trsmstr = Trans_master::orderBy('trans_id', 'ASC')->get();
        $perkiraan = Perkiraan::all();
        $tgllogin = Mysysid::where('KeyName', '=', 'TANGGALHARIINI')->get();
        $tgllogin = date('Y-m-d', strtotime(str_replace('/', '-', $tgllogin[0]->Value)));

        return view('akuntansi.frmhistorycatatjurnal', ['users' => $users, 'logos' => $logos, 'history' => $history, 'trsmstr' => $trsmstr, 'perkiraan' => $perkiraan, 'tgl_trans' => $tgllogin, 'msgstatus' => '']);
    }
    // GET HISTORY JURNAL PAKE JSON
    public function getJurnals(Request $request)
    {
        // dd($request);
        ## Read value
        ...
        // Total records
        $totalRecords = Trans_master::select('count(*) as allcount')->count();
        $totalRecordswithFilter = Trans_master::select('trans_id', 'tgl_trans', 'kode_jurnal', 'no_bukti', 'src')
        ...
        foreach ($records as $record) {
            ...
            $tgltrans = $record->tgl_trans;
            $kodejurnal = $record->kode_jurnal;
    ...
    public function bo_ak_tt_detailhistorycatatjurnal(Request $request)
    {
        // dd($request);
        $users = User::all();
        $logos = Logo::all();
        $perkiraan = Perkiraan::orderBy('kode_perk', 'ASC')->get();
        $transdetail = Trans_detail::with('perkiraan')->where('master_id', $request->trans_id)->orderBy('trans_id')->get();
        $kodejurnal = KodeJurnal::all();
        $tgllogin = Mysysid::where('KeyName', '=', 'TANGGALHARIINI')->get();
        $tgllogin = date('Y-m-d', strtotime(str_replace('/', '-', $tgllogin[0]->Value)));

        return view('akuntansi.frmhistorycatatjurnal', ['users' => $users, 'logos' => $logos, 'perkiraan' => $perkiraan, 'master_id' => $request->trans_id, 'kode_jurnal' => $request->kode_jurnal, 'tgl_trans' => $request->tgl_trans, 'transdetail' => $transdetail, 'kodejurnal' => $kodejurnal, 'no_bukti' => $request->no_bukti, 'tgllogin' => $tgllogin, 'msgstatus' => '']);
    }
    ...
    public function bo_ak_tt_updatehistorycatatjurnal(Request $request)
    {
        ...
        $transId = 0;
        if (isset($request->xmaster_id)) {
            $simpanupdate = new Trans_detail();
            $simpanupdate->master_id = $request->master_id;
            $simpanupdate->URAIAN = $request->keterangan;
            $simpanupdate->kode_perk = $request->kode_perk;
            $simpanupdate->debet = $request->debet;
            $simpanupdate->kredit = $request->kredit;
            $simpanupdate->save();
            $transId = $simpanupdate->trans_id;
        } else {
        ...
        // PROSES UPDATE SALDO_AKHIR TRANS_DETAIL
        $saldo_awal = (float)$request->saldo_awal;
        if ($request->dk == 'D') {
            $saldo_akhir = DB::select("SELECT trans_detail.kode_perk,($saldo_awal+(SUM(trans_detail.debet) - SUM(trans_detail.kredit))) as saldo_akhir FROM trans_master INNER JOIN trans_detail ON trans_master.trans_id=trans_detail.master_id WHERE trans_master.tgl_trans<='$request->inputtgljurnal' AND trans_detail.kode_perk='$request->kode_perk' GROUP BY trans_detail.kode_perk")[0]->saldo_akhir;
        } else {
            $saldo_akhir = DB::select("SELECT trans_detail.kode_perk,($saldo_awal+(SUM(trans_detail.kredit) - SUM(trans_detail.debet))) as saldo_akhir FROM trans_master INNER JOIN trans_detail ON trans_master.trans_id=trans_detail.master_id WHERE trans_master.tgl_trans<='$request->inputtgljurnal' AND trans_detail.kode_perk='$request->kode_perk' GROUP BY trans_detail.kode_perk")[0]->saldo_akhir;
        }
    ...
    {
        // dd($request);
        Trans_detail::where('trans_id', $request->trans_id)->delete();;

        // $trd->delete();
        return redirect()->route('historycatatjurnal', ['trans_id' => $request->master_id, 'kode_jurnal' => $request->kode_jurnal, 'no_bukti' => $request->no_bukti, 'tgl_trans' => $request->tgl_trans])->with('alert', 'Delete TRANS_ID : ' . $request->trans_id . ' Berhasil');
    }
