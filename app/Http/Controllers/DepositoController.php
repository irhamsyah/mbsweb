...
use App\Exports\ReportMutasiBngDepExport;
use App\Exports\ReportDepositoBlokirExport;
use App\Exports\ReportOBBungaToTitpanExport;
use App\Exports\ReportBungaPajakDepExport;
use App\Exports\ReportJadwalBayarBungaExport;
use App\Exports\ReportDepositoblmAktifExport;
use App\Exports\ReportDepositoJTExport;
...
class DepositoController extends Controller
{
        ...
        if (md5($request->eno_rekening . 'Bast90') == $request->eno_rekeningHashedit) {
            ...
            if ($request->eno_rekening != '' && $request->ejenis_deposito != '' && $request->ejml_deposito != '' && $request->etgl_registrasi != '' && $request->einputstatus != '') {
                ...
                Deposito::where('NO_REKENING', $request->eno_rekening)
                    ->update([
                        'NO_ALTERNATIF' => $request->eno_bilyet,
    ...
    public function bo_dp_de_hitungbungadep()
    {
        $logos = Logo::all();
        $users = User::all();
    ...
    public function bo_dp_de_hitungbungadep_view(Request $request)
    {
        $cekadabunga = DB::select("SELECT count(*) hit FROM deposito WHERE STATUS_AKTIF='2' AND BUNGA_BLN_INI>0 ORDER BY TGL_REGISTRASI DESC");
        if ($cekadabunga[0]->hit > 0) {
            return redirect()->route('showhitungbungadep')->with('alert', 'MASIH ADA DSPOSITO YG BELOM DIPERPANJANG/DIAMBIL');
        }
    ...
    //show form browse bunga and pajak deposito 
    public function bo_dp_de_browsebunga()
    {
        $logos = Logo::all();
        $users = User::all();
        $brwsebngpjk = DB::select("select deposito.*,nasabah.nama_nasabah,nasabah.alamat from deposito inner join nasabah on deposito.nasabah_id=nasabah.nasabah_id where deposito.status_aktif=2");
    ...
    public function bo_dp_rp_nominatifrinci()
    {
        ...
        return view(
            'reports.deposito.frmnominatifdep',
    ...
    public function bo_dp_rp_nominatifgroup()
    {
        ...
        return view(
            'reports.deposito.frmnominatifgroup',
            ['users' => $users, 'logos' => $logos, 'tgllogin' => $tgllogin]
        );
    }
    ...
    public function cetaknominatifgrouprinci()
    {
        ...
        $logos = Logo::all();
        return view(
            'reports.deposito.frmnominatifgrouprinci',
    ...
    public function bo_dp_rp_mutasibunga_view(Request $request)
    {
        ...
        $transaksi = DB::select("SELECT DEPTRANS_ID,deptrans.NO_REKENING,deptrans.TGL_TRANS,deposito.TITIPAN_AWAL,deptrans.SALDO_TRANS,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0) as terima_pokok,IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%' OR MY_KODE_TRANS LIKE '45%' OR MY_KODE_TRANS='400',SALDO_TRANS,0) as setor,IF(MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '4%' OR MY_KODE_TRANS LIKE '3%' OR MY_KODE_TRANS LIKE '25%' OR MY_KODE_TRANS ='200',SALDO_TRANS,0) as ambil,KUITANSI,deptrans.my_kode_trans,no_rek_ob,tob FROM deposito INNER JOIN deptrans ON deposito.NO_REKENING=deptrans.NO_REKENING WHERE deptrans.NO_REKENING='$request->no_rekening' AND TGL_TRANS>='$tgltrs1' AND TGL_TRANS<='$tgltrs2' AND MY_KODE_TRANS<>1 ORDER BY DEPTRANS_ID");
    ...
    public function exporttoexcelmutasibngdep(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $tgltrs2 = date('Y-m-d', strtotime($request->tgl_trans2));
        $transaksi = DB::select("SELECT DEPTRANS_ID,deptrans.NO_REKENING,deptrans.TGL_TRANS,deposito.TITIPAN_AWAL,deptrans.SALDO_TRANS,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0) as terima_pokok,IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%' OR MY_KODE_TRANS LIKE '45%' OR MY_KODE_TRANS='400',SALDO_TRANS,0) as setor,IF(MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '4%' OR MY_KODE_TRANS LIKE '3%' OR MY_KODE_TRANS LIKE '25%' OR MY_KODE_TRANS ='200',SALDO_TRANS,0) as ambil,KUITANSI,deptrans.my_kode_trans,no_rek_ob,tob FROM deposito INNER JOIN deptrans ON deposito.NO_REKENING=deptrans.NO_REKENING WHERE deptrans.NO_REKENING='$request->no_rekening' AND TGL_TRANS>='$tgltrs1' AND TGL_TRANS<='$tgltrs2' AND MY_KODE_TRANS<>1 ORDER BY DEPTRANS_ID");

        return (new ReportMutasiBngDepExport($transaksi))->download('mutasibunga.xlsx');
    }
    // Show Form Deposito Blokir
    public function bo_dp_rp_depositoblokir()
    {
        $users = User::all();
        $logos = Logo::all();
        return view('reports.deposito.frmrptdepblokir', ['users' => $users, 'logos' => $logos, 'msgstatus' => '']);
    }
    // Proses Cari 
    public function bo_dp_rp_depositoblokir_view(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required'
        ]);
        ...
        $users = User::all();
        $logos = Logo::all();
        $tgltrs = date('Y-m-d', strtotime($request->tgl_trans1));
        $blokirdep = DB::select("SELECT deposito.KODE_GROUP1,kodegroup1deposito.DESKRIPSI_GROUP1,deposito.KODE_GROUP2,kodegroup2deposito.DESKRIPSI_GROUP2,deposito.KODE_GROUP3,kodegroup3deposito.DESKRIPSI_GROUP3,nasabah.nasabah_id,deposito.NO_REKENING,CONCAT(nasabah.NAMA_KUASA,'',deposito.QQ) as nama_nasabah,nasabah.alamat,deposito.TGL_REGISTRASI,deposito.SUKU_BUNGA,deposito.TGL_VALUTA,X.jml_deposito,deposito.jkw,deposito.TGL_JT,deposito.JENIS_DEPOSITO,deposito.BLOKIR,deposito.TGL_BLOKIR,deposito.KODE_BI_PEMILIK,deposito.SUKU_BUNGA,deposito.BUNGA_BLN_INI,deposito.PAJAK_BLN_INI FROM ((((deposito LEFT JOIN kodegroup1deposito ON deposito.KODE_GROUP1=kodegroup1deposito.KODE_GROUP1) LEFT JOIN kodegroup2deposito ON deposito.KODE_GROUP2=kodegroup2deposito.KODE_GROUP2) LEFT JOIN kodegroup3deposito ON deposito.KODE_GROUP3=kodegroup3deposito.KODE_GROUP3) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS LIKE '3%',SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs' GROUP BY NO_REKENING) as X ON deposito.NO_REKENING=X.NO_REKENING) INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id WHERE deposito.BLOKIR=1 AND deposito.ABP<>3 AND deposito.TGL_MULAI<='$tgltrs'");

        return view('reports.deposito.frmrptdepblokir', ['users' => $users, 'logos' => $logos, 'tgltrs' => $tgltrs, 'blokirdep' => $blokirdep]);
    }
    // CETAK PDF DEPOSITO BLOKIR
    public function bo_dp_rp_cetakdepositoblokir(Request $request)
    {
        $tgltrs = date('Y-m-d', strtotime($request->tgl_trans1));

        $blokirdep = DB::select("SELECT deposito.KODE_GROUP1,kodegroup1deposito.DESKRIPSI_GROUP1,deposito.KODE_GROUP2,kodegroup2deposito.DESKRIPSI_GROUP2,deposito.KODE_GROUP3,kodegroup3deposito.DESKRIPSI_GROUP3,nasabah.nasabah_id,deposito.NO_REKENING,CONCAT(nasabah.nama_nasabah,'',deposito.QQ) as nama_nasabah,nasabah.alamat,deposito.TGL_REGISTRASI,deposito.SUKU_BUNGA,deposito.TGL_VALUTA,X.jml_deposito,deposito.jkw,deposito.TGL_JT,deposito.JENIS_DEPOSITO,deposito.BLOKIR,deposito.TGL_BLOKIR,deposito.KODE_BI_PEMILIK,deposito.SUKU_BUNGA,deposito.TGL_MULAI FROM ((((deposito LEFT JOIN kodegroup1deposito ON deposito.KODE_GROUP1=kodegroup1deposito.KODE_GROUP1) LEFT JOIN kodegroup2deposito ON deposito.KODE_GROUP2=kodegroup2deposito.KODE_GROUP2) LEFT JOIN kodegroup3deposito ON deposito.KODE_GROUP3=kodegroup3deposito.KODE_GROUP3) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS LIKE '3%',SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs' GROUP BY NO_REKENING) as X ON deposito.NO_REKENING=X.NO_REKENING) INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id WHERE deposito.BLOKIR=1 AND deposito.ABP<>3 AND deposito.TGL_BLOKIR<='$tgltrs'");
        $lembaga = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'NAMA_LEMBAGA' . '%')->get();
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.depositoblokir_pdf', ['blokirdep' => $blokirdep, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota, 'tgltrs' => $tgltrs])->setPaper('A4', 'landscape');
        return $pdf->stream();
    }
    // EXPORT TO EXCEL DEPOSITO BLOKIR 
    public function exportdepositoblokir(Request $request)
    {
        $tgltrs = date('Y-m-d', strtotime($request->tgl_trans1));

        $blokirdep = DB::select("SELECT deposito.KODE_GROUP1,kodegroup1deposito.DESKRIPSI_GROUP1,deposito.KODE_GROUP2,kodegroup2deposito.DESKRIPSI_GROUP2,deposito.KODE_GROUP3,kodegroup3deposito.DESKRIPSI_GROUP3,nasabah.nasabah_id,deposito.NO_REKENING,CONCAT(nasabah.nama_nasabah,'',deposito.QQ) as nama_nasabah,nasabah.alamat,deposito.TGL_REGISTRASI,deposito.SUKU_BUNGA,deposito.TGL_VALUTA,X.jml_deposito,deposito.jkw,deposito.TGL_JT,deposito.JENIS_DEPOSITO,deposito.BLOKIR,deposito.TGL_BLOKIR,deposito.KODE_BI_PEMILIK,deposito.TGL_MULAI FROM ((((deposito LEFT JOIN kodegroup1deposito ON deposito.KODE_GROUP1=kodegroup1deposito.KODE_GROUP1) LEFT JOIN kodegroup2deposito ON deposito.KODE_GROUP2=kodegroup2deposito.KODE_GROUP2) LEFT JOIN kodegroup3deposito ON deposito.KODE_GROUP3=kodegroup3deposito.KODE_GROUP3) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS LIKE '3%',SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs' GROUP BY NO_REKENING) as X ON deposito.NO_REKENING=X.NO_REKENING) INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id WHERE deposito.BLOKIR=1 AND deposito.ABP<>3 AND deposito.TGL_BLOKIR<='$tgltrs'");

        return (new ReportDepositoBlokirExport($blokirdep))->download('depositoblokir.xlsx');
    }
    // FORM SHOW CARI TRANSAKSI BUNGA KE TITIPAN 
    public function bbo_dp_rp_obbungaketitipan()
    {
        $users = User::all();
        $logos = Logo::all();
        return view('reports.deposito.frmobbngkettpan', ['users' => $users, 'logos' => $logos, 'msgstatus' => '']);
    }
    // Proses bung
    public function bbo_dp_rp_obbungaketitipan_view(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required',
            'tgl_trans2' => 'required'
        ...
        $transaksi = DB::select("SELECT deposito.no_rekening,nasabah.nama_nasabah,deposito.suku_bunga,Y.TGL_TRANS,Y.kode_trans,Y.terima_pokok,Y.ob_bunga,Y.ambil_titipan,Y.ob_pajak,Y.ambil_pokok,Y.kuitansi,Y.no_teller,Y.userid,Y.my_kode_trans,deposito.KODE_GROUP1,deposito.KODE_GROUP2,deposito.KODE_GROUP3,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,Y.tob,Y.no_rek_ob FROM (deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id ) INNER JOIN (SELECT DEPTRANS_ID,NO_REKENING,TGL_TRANS,SUM(IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0)) as terima_pokok,SUM(IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%',SALDO_TRANS,0)) as ob_bunga,SUM(IF(MY_KODE_TRANS LIKE '27%',SALDO_TRANS,0)) as ambil_titipan,SUM(IF(MY_KODE_TRANS LIKE '4%',SALDO_TRANS,0)) as ob_pajak,SUM(IF(MY_KODE_TRANS LIKE '3%',SALDO_TRANS,0)) as ambil_pokok,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',KUITANSI,NULL) as kuitansi,no_teller,userapp as userid,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',MY_KODE_TRANS,NULL) as my_kode_trans,no_rek_ob,tob,kode_trans FROM deptrans WHERE TGL_TRANS>='$tgltrs1' AND TGL_TRANS<='$tgltrs2' AND (deptrans.MY_KODE_TRANS LIKE '%75' OR deptrans.MY_KODE_TRANS='100') GROUP BY NO_REKENING,TGL_TRANS ORDER BY TGL_TRANS) AS Y ON deposito.NO_REKENING=Y.NO_REKENING");

        return view('reports.deposito.frmobbngkettpan', ['users' => $users, 'logos' => $logos, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2, 'transaksi' => $transaksi]);
    }
    // CETAK PDF OB BUNGA KE TITIPAN
    public function bo_dp_rp_cetakobbungaketitipan(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required',
            'tgl_trans2' => 'required'
        ...
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.obbungatottpan_pdf', ['transaksi' => $transaksi, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2])->setPaper('A4', 'portrait');
        return $pdf->stream();
    }
    // export to excel OB BUNGA TO Titipan
    public function exportobbngtotitipan(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required',
            'tgl_trans2' => 'required'
        ...
        $tgltrs2 = date('Y-m-d', strtotime($request->tgl_trans2));
        $transaksi = DB::select("SELECT deposito.no_rekening,nasabah.nama_nasabah,deposito.suku_bunga,Y.TGL_TRANS,Y.kode_trans,Y.terima_pokok,Y.ob_bunga,Y.ambil_titipan,Y.ob_pajak,Y.ambil_pokok,Y.kuitansi,Y.no_teller,Y.userid,Y.my_kode_trans,deposito.KODE_GROUP1,deposito.KODE_GROUP2,deposito.KODE_GROUP3,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,Y.tob,Y.no_rek_ob FROM (deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id ) INNER JOIN (SELECT DEPTRANS_ID,NO_REKENING,TGL_TRANS,SUM(IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0)) as terima_pokok,SUM(IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%',SALDO_TRANS,0)) as ob_bunga,SUM(IF(MY_KODE_TRANS LIKE '27%',SALDO_TRANS,0)) as ambil_titipan,SUM(IF(MY_KODE_TRANS LIKE '4%',SALDO_TRANS,0)) as ob_pajak,SUM(IF(MY_KODE_TRANS LIKE '3%',SALDO_TRANS,0)) as ambil_pokok,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',KUITANSI,NULL) as kuitansi,no_teller,userapp as userid,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',MY_KODE_TRANS,NULL) as my_kode_trans,no_rek_ob,tob,kode_trans FROM deptrans WHERE TGL_TRANS>='$tgltrs1' AND TGL_TRANS<='$tgltrs2' AND (deptrans.MY_KODE_TRANS LIKE '%75' OR deptrans.MY_KODE_TRANS='100') GROUP BY NO_REKENING,TGL_TRANS ORDER BY TGL_TRANS) AS Y ON deposito.NO_REKENING=Y.NO_REKENING");
        return (new ReportOBBungaToTitpanExport($transaksi, $users))->download('exportbungatotitipan.xlsx');
    }
    // Show form pencarian bunga dan pajak
    public function bo_dp_rp_bungapajakdep()
    {
        $users = User::all();
        $logos = Logo::all();
        return view('reports.deposito.frmbungapajakdep', ['users' => $users, 'logos' => $logos, 'msgstatus' => '']);
    }
    // Proses Cari Bunga Pajak
    public function bo_dp_rp_bungapajakdep_view(Request $request)
    {
        $month = date('m', strtotime($request->tahun . '-' . substr($request->bulan, 0, 2) . '-' . substr($request->bulan, 2, 4)));
        $bulan = "";
        $bulanX = $request->bulan;
        switch ($month) {
            case 1:
                $bulan = "Januari";
                break;
        ...
        $tgltrs2 = date('Y-m-d', strtotime($request->tahun . '-' . substr($request->bulan, 0, 2) . '-' . substr($request->bulan, 2, 4)));
        $x = (int)substr($request->bulan, 2, 4);
        $tgltrs1 = date('Y-m-d', strtotime("-" . $x . " days", strtotime($tgltrs2)));
        $users = User::all();
        $logos = Logo::all();
        $transaksi = DB::select("SELECT deposito.no_rekening,nasabah.nama_nasabah,deposito.suku_bunga,deposito.jml_deposito,deposito.tgl_registrasi,Y.TGL_TRANS,Y.kode_trans,Y.terima_pokok,Y.bunga,Y.titipan,Y.pajak,Y.kuitansi,Y.no_teller,Y.userid,deposito.KODE_GROUP1,deposito.KODE_GROUP2,deposito.KODE_GROUP3,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,Y.tob,Y.no_rek_ob FROM ((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id ) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) LEFT JOIN (SELECT DEPTRANS_ID,NO_REKENING,TGL_TRANS,SUM(IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0)) as terima_pokok,SUM(IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%',SALDO_TRANS,0)) as bunga,SUM(IF(MY_KODE_TRANS LIKE '27%',SALDO_TRANS,0)) as titipan,SUM(IF(MY_KODE_TRANS LIKE '4%',SALDO_TRANS,0)) as pajak,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',KUITANSI,NULL) as kuitansi,no_teller,userapp as userid,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',MY_KODE_TRANS,NULL) as my_kode_trans,no_rek_ob,tob,kode_trans FROM deptrans WHERE TGL_TRANS>'$tgltrs1' AND TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING ORDER BY TGL_TRANS) AS Y ON deposito.NO_REKENING=Y.NO_REKENING");
        return view('reports.deposito.frmbungapajakdep', ['users' => $users, 'logos' => $logos, 'transaksi' => $transaksi, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2, 'bulan' => $bulan, 'bulanX' => $request->bulan]);
    }
    // Cetak PDF Bunga Pajak
    public function bo_dp_rp_cetakbngpjkdeposito(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $tgltrs2 = date('Y-m-d', strtotime($request->tgl_trans2));

        $transaksi = DB::select("SELECT deposito.no_rekening,nasabah.nama_nasabah,deposito.suku_bunga,deposito.jml_deposito,deposito.tgl_registrasi,Y.TGL_TRANS,Y.kode_trans,Y.terima_pokok,Y.bunga,Z.saldo_titipan,Y.pajak,Y.kuitansi,Y.no_teller,Y.userid,deposito.KODE_GROUP1,deposito.KODE_GROUP2,deposito.KODE_GROUP3,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,Y.tob,Y.no_rek_ob FROM (((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id ) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) LEFT JOIN (SELECT DEPTRANS_ID,NO_REKENING,TGL_TRANS,SUM(IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0)) as terima_pokok,SUM(IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%',SALDO_TRANS,0)) as bunga,SUM(IF(MY_KODE_TRANS LIKE '4%',SALDO_TRANS,0)) as pajak,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',KUITANSI,NULL) as kuitansi,no_teller,userapp as userid,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',MY_KODE_TRANS,NULL) as my_kode_trans,no_rek_ob,tob,kode_trans FROM deptrans WHERE TGL_TRANS>'$tgltrs1' AND TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING ORDER BY TGL_TRANS) AS Y ON deposito.NO_REKENING=Y.NO_REKENING) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS LIKE '10%' AND TGL_TRANS<='$tgltrs1',SALDO_TRANS,0))-SUM(IF((MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '47%') AND (TGL_TRANS<='$tgltrs1'),SALDO_TRANS,0))) as saldo_titipan FROM deptrans GROUP BY NO_REKENING) as Z ON deposito.NO_REKENING=Z.NO_REKENING");
        $lembaga = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'NAMA_LEMBAGA' . '%')->get();
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.bungapajak_pdf', ['transaksi' => $transaksi, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2])->setPaper('A4', 'portrait');
        return $pdf->stream();
    }
    // EXPORT BUNGA PAJAK TO EXCEL
    public function exportbungapajakdep(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $tgltrs2 = date('Y-m-d', strtotime($request->tgl_trans2));

        $transaksi = DB::select("SELECT deposito.no_rekening,nasabah.nama_nasabah,nasabah.alamat,deposito.jml_deposito,deposito.tgl_registrasi,Y.TGL_TRANS,Y.kode_trans,Y.terima_pokok,Y.bunga,Z.saldo_titipan,Y.pajak,Y.kuitansi,Y.no_teller,Y.userid,deposito.KODE_GROUP1,deposito.KODE_GROUP2,deposito.KODE_GROUP3,deposito.MASUK_TITIPAN,deposito.BUNGA_BERBUNGA,Y.tob,Y.no_rek_ob FROM (((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id ) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))) AS jml_deposito FROM deptrans WHERE TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) LEFT JOIN (SELECT DEPTRANS_ID,NO_REKENING,TGL_TRANS,SUM(IF(MY_KODE_TRANS LIKE '0',SALDO_TRANS,0)) as terima_pokok,SUM(IF(MY_KODE_TRANS LIKE '10%' OR MY_KODE_TRANS LIKE '20%' OR MY_KODE_TRANS LIKE '25%',SALDO_TRANS,0)) as bunga,SUM(IF(MY_KODE_TRANS LIKE '4%',SALDO_TRANS,0)) as pajak,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',KUITANSI,NULL) as kuitansi,no_teller,userapp as userid,IF(MY_KODE_TRANS = '0' OR MY_KODE_TRANS = '100' OR MY_KODE_TRANS ='200' OR MY_KODE_TRANS ='300',MY_KODE_TRANS,NULL) as my_kode_trans,no_rek_ob,tob,kode_trans FROM deptrans WHERE TGL_TRANS>'$tgltrs1' AND TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING ORDER BY TGL_TRANS) AS Y ON deposito.NO_REKENING=Y.NO_REKENING) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS LIKE '10%' AND TGL_TRANS<='$tgltrs1',SALDO_TRANS,0))-SUM(IF((MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '47%') AND (TGL_TRANS<='$tgltrs1'),SALDO_TRANS,0))) as saldo_titipan FROM deptrans GROUP BY NO_REKENING) as Z ON deposito.NO_REKENING=Z.NO_REKENING");

        return (new ReportBungaPajakDepExport($transaksi))->download('exportbungapajak.xlsx');
    }
    // Show form Cetak jadwal bunga Pajak
    public function bo_dp_rp_frmjadwaldeposito()
    {
        $users = User::all();
        $logos = Logo::all();
        return view('reports.deposito.frmcetakjadwalbunga', ['users' => $users, 'logos' => $logos, 'msgstatus' => '']);
    }
    // Proses Cari Jadwal Bayar Deposito
    public function bo_dp_rp_frmjadwaldeposito_view(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required',
            'tgl_trans2' => 'required'
        ...
        $users = User::all();
        $logos = Logo::all();
        $transaksi = DB::select("SELECT deposito.*,nasabah.nama_nasabah,X.nominal,Y.saldo_titipan FROM ((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS=300,SALDO_TRANS,0))) AS nominal FROM deptrans WHERE TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS LIKE '10%' AND TGL_TRANS<'$tgltrs1',SALDO_TRANS,0))-SUM(IF((MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '47%') AND (TGL_TRANS<'$tgltrs1'),SALDO_TRANS,0))) as saldo_titipan FROM deptrans GROUP BY NO_REKENING) as Y ON deposito.NO_REKENING=Y.NO_REKENING WHERE X.nominal>0 AND bunga_bln_ini>0");
        return view('reports.deposito.frmcetakjadwalbunga', ['users' => $users, 'logos' => $logos, 'transaksi' => $transaksi, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2]);
    }
    // Cetak ke PDF Jadwal Bayar Bunga
    public function bo_dp_rp_cetakjadwalbunga(Request $request)
    {
        $this->validate($request, [
            'tgl_trans1' => 'required',
            'tgl_trans2' => 'required'
        ...
        $transaksi = DB::select("SELECT deposito.*,nasabah.nama_nasabah,X.nominal,Y.saldo_titipan FROM ((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS=300,SALDO_TRANS,0))) AS nominal FROM deptrans WHERE TGL_TRANS<='$tgltrs2' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) LEFT JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS LIKE '10%' AND TGL_TRANS<'$tgltrs1',SALDO_TRANS,0))-SUM(IF((MY_KODE_TRANS LIKE '27%' OR MY_KODE_TRANS LIKE '47%') AND (TGL_TRANS<'$tgltrs1'),SALDO_TRANS,0))) as saldo_titipan FROM deptrans GROUP BY NO_REKENING) as Y ON deposito.NO_REKENING=Y.NO_REKENING WHERE X.nominal>0 AND bunga_bln_ini>0 ORDER BY deposito.TGL_VALUTA");
        $lembaga = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'NAMA_LEMBAGA' . '%')->get();
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.jadwalbyrbunga_pdf', ['transaksi' => $transaksi, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota, 'tgltrs1' => $tgltrs1, 'tgltrs2' => $tgltrs2])->setPaper('A4', 'landscape');
        return $pdf->stream();
    }
    // export to excel jadwal bayar bunga
    public function exportjadwalbyrbng(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $tgltrs2 = date('Y-m-d', strtotime($request->tgl_trans2));
        ...
        return (new ReportJadwalBayarBungaExport($transaksi, $tgltrs1))->download('jadwalbayarbunga.xlsx');
    }
    // Show Form Deposito Belum AKtif
    public function bo_dp_rp_depositoblumaktif()
    {
        $users = User::all();
        $logos = Logo::all();
        $belumaktif = DB::select("Select deposito.*,nasabah.nama_nasabah,nasabah.alamat from deposito inner join nasabah on deposito.nasabah_id=nasabah.nasabah_id  where status_aktif=1");
        return view('reports.deposito.frmcdepositoblmaktif', ['users' => $users, 'logos' => $logos, 'belumaktif' => $belumaktif]);
    }
    // Cetak PDF Deposito Belum AKtif
    public function bo_dp_rp_cetakbelumaktif(Request $request)
    {
        $belumaktif = DB::select("Select deposito.*,nasabah.nama_nasabah,nasabah.alamat from deposito inner join nasabah on deposito.nasabah_id=nasabah.nasabah_id  where status_aktif=1");
        $lembaga = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'NAMA_LEMBAGA' . '%')->get();
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.depositobelumaktif_pdf', ['belumaktif' => $belumaktif, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota])->setPaper('A4', 'landscape');
        return $pdf->stream();
    }
    // export deposito belom aktif
    public function bo_dp_rp_exportbelumaktif(Request $request)
    {
        $belumaktif = DB::select("Select deposito.*,nasabah.nama_nasabah,nasabah.alamat from deposito inner join nasabah on deposito.nasabah_id=nasabah.nasabah_id  where status_aktif=1");
        return (new ReportDepositoblmAktifExport($belumaktif))->download('depositobelumaktif.xlsx');
    }
    // SHow form Cari Deposito Jatuh Tempo
    public function bo_dp_rp_depositojttmp()
    {
        $users = User::all();
        $logos = Logo::all();

        return view('reports.deposito.frmcetakjatuhtempo', ['users' => $users, 'logos' => $logos]);
    }
    // PROSES CARI DEPOSITO JATUH TEMPO
    public function bo_dp_rp_depositojttmp_view(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $users = User::all();
        $logos = Logo::all();

        $transaksi = DB::select("SELECT deposito.*,nasabah.nama_nasabah,nasabah.alamat,X.nominal FROM (deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS=300,SALDO_TRANS,0))) AS nominal FROM deptrans WHERE TGL_TRANS<='$tgltrs1' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING WHERE X.nominal>0 AND deposito.TGL_JT<='$tgltrs1'");
        return view('reports.deposito.frmcetakjatuhtempo', ['users' => $users, 'logos' => $logos, 'transaksi' => $transaksi, 'tgltrs1' => $tgltrs1]);
    }
    // Cetak PDF JATUH TEMPO DEPOSITO 
    public function bo_dp_rp_cetakjatuhtempo(Request $request)
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $transaksi = DB::select("SELECT deposito.*,nasabah.nama_nasabah,nasabah.alamat,X.nominal FROM (deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS=300,SALDO_TRANS,0))) AS nominal FROM deptrans WHERE TGL_TRANS<='$tgltrs1' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING WHERE X.nominal>0 AND deposito.TGL_JT<='$tgltrs1'");
        $lembaga = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'NAMA_LEMBAGA' . '%')->get();
        $ttd = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'like', 'TTD_DEP%')->get();
        $kota = DB::table('mysysid')->select('KeyName', 'Value')->where('KeyName', 'NAMA_KOTA')->get();
        $pdf = Pdf::loadview('pdf.deposito.jatuhtempo_pdf', ['transaksi' => $transaksi, 'tgltrs1' => $tgltrs1, 'lembaga' => $lembaga, 'ttd' => $ttd, 'kota' => $kota])->setPaper('A4', 'landscape');
        return $pdf->stream();
    }
    // EXPORT TO EXCEL
    ...
    {
        $tgltrs1 = date('Y-m-d', strtotime($request->tgl_trans1));
        $transaksi = DB::select("SELECT deposito.*,nasabah.nama_nasabah,nasabah.alamat,jenis_kota.Deskripsi_Kota,X.nominal,kodejenisdeposito.DESKRIPSI_JENIS_DEPOSITO FROM (((deposito INNER JOIN nasabah ON deposito.NASABAH_ID=nasabah.nasabah_id) INNER JOIN (SELECT NO_REKENING,(SUM(IF(MY_KODE_TRANS=0 OR MY_KODE_TRANS=1,SALDO_TRANS,0))-SUM(IF(MY_KODE_TRANS=300,SALDO_TRANS,0))) AS nominal FROM deptrans WHERE TGL_TRANS<='$tgltrs1' GROUP BY NO_REKENING) AS X ON deposito.NO_REKENING=X.NO_REKENING) INNER JOIN jenis_kota ON nasabah.kota_id=jenis_kota.kota_id) INNER JOIN kodejenisdeposito ON deposito.JENIS_DEPOSITO=kodejenisdeposito.KODE_JENIS_DEPOSITO WHERE X.nominal>0 AND deposito.TGL_JT<='$tgltrs1'");
        return (new ReportDepositoJTExport($transaksi))->download('depositojatuhtempo.xlsx');
    }
}
