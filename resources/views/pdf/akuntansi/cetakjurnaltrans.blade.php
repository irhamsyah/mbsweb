...
<html lang="en">
    ...
    <div class="container" style="border: 0px">
      ...
      <a href="{{route('showfrmrptjurnaltransaksi')}}" class="btn btn-md btn-normal">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-house-check" viewBox="0 0 16 16">
          ...
          <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm1.679-4.493-1.335 2.226a.75.75 0 0 1-1.174.144l-.774-.773a.5.5 0 0 1 .708-.707l.547.547 1.17-1.951a.5.5 0 1 1 .858.514Z"/>
            ...
            <h4 class="text-left">Jurnal Transaksi</h4>
            @foreach($lembaga as $lembaga)
            <br>{{$lembaga->Value}}<br>
            @endforeach

          <table id="tbl1" class="table table-bordered table-responsive" style="border-style: none;position:relative">
            <thead>
            <tr>
            ...
            <tbody>
              @php($total_debet=0)
              @php($total_kredit=0)
            @foreach(array_chunk($jurnal,1) as $key)
              @foreach($key as $value=>$keys)
                <tr>
                    <td>{{ $keys->tgl_trans }}</td>
                    <td>{{ $keys->kode_perk }}</td>
                    <td>{{ $keys->nama_perk }}</td>
                    <td>{{ $keys->URAIAN }}</td>
                    <td>{{ number_format($keys->debet,2)}}</td>
                    <td>{{ number_format($keys->kredit,2)}}</td>
                    @php($total_debet=$total_debet+$keys->debet)
                    @php($total_kredit=$total_kredit+$keys->kredit)
                </tr>
              @endforeach
            @endforeach
            <tr>
              <td colspan="4"><h5>Jumlah</h5></td>
              <td>{{ number_format($total_debet,2)}}</td>
              <td>{{ number_format($total_kredit,2)}}</td>
            </tr>
            </tbody>
        </table>
        <div class="row" style="margin-left:51%">{{date('d-m-Y')}}</div>
