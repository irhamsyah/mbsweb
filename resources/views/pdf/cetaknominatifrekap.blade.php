            ...
            @foreach(array_chunk($nominatif,1) as $key)
              @foreach($key as $value=>$keys)
                <tr>
                    ...
                    <td>{{ number_format($keys->aggregate)}}</td>
                    <td>{{ number_format($keys->saldo_nominatif,2)}}</td>
                    <td>{{ $keys->persennisbah}}</td>
                    <td>{{ $keys->persensaldo}}</td>
                </tr>
                @php($aggregate=$aggregate+$keys->aggregate)
                @php($saldo_nominatif=$saldo_nominatif+$keys->saldo_nominatif)
                @php($persennisbah=$keys->persennisbah)
                @php($persensaldo=$keys->persensaldo)
              @endforeach
            @endforeach
            <tr>
              <td colspan="2"><h5>Total Keseluruhan</h5></td>
              <td>{{ number_format($aggregate)}}</td>
