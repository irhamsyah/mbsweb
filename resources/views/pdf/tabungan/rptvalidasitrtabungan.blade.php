...
<body>
    <div class="container">
        ...
        <a href="{{route('setoranpenarikantabungan')}}" class="btn btn-md btn-normal">
        ...
        <div class="container" style="margin-left: 150px;font-size:11pt">
            <div class="row" style="padding: 0px;margin-bottom: -8px;">
                ...
                {{date('d/m/Y H:i:s')}}
                {{Auth::user()->username}}
                @php($pjg=strlen($kode_trans))
                {{substr($kode_trans,0,2).'-'.substr($kode_trans,7,$pjg)}}
            </div>
            <div class="row" style="padding: 0px;margin-bottom: -8px;">
                {{$no_rekening}}
                {{$nama_nasabah}}
            ...
            <div class="row" style="padding: 0px;margin-bottom: -8px;">
                {{$kuitansi}}
                @if(substr($kode_trans,5,1)=='K')
                CR
                @else
                DB
                @endif
                {{number_format((float)$setorambil,2,".",",")}}
